...
| 梦中的你 | 怀念：回合开始赋予一张手牌保留并禁用本回合使用；噩梦：基于历史使用次数禁用最常用计策，不会在第一次使用时立刻禁用 |
| 盗贼 | 遗物偷取：随机一件遗物效果在本场战斗失效；顺手的事：玩家每回合减少 1 金币 |
...
| 留场 | 回合结束 / 下回合开始 | 本牌继续留在倍率格，不进入弃牌堆；下回合开始移除本牌留场词条 |
| 蔓延 | 打出时 | 将 0 点扩散牌加入手牌 |
...
大数字体系中，`学徒铸造` / `大师铸造` 与 `预借` / `熟练预借` 使用 `battleBonus` 表示“本场/本次战斗永久”点数变化，战斗结束后统一清零；跨战斗成长仍只写入 `permanentBonus`。
...
| 数值强化 | 目标单卡永久 +5，2 金币起，费用按单卡独立递增 |
| 购买遗物 | 3 件可选，低/中/高按 65%/30%/5% 刷新，优先使用遗物详细配置价格 |
| 购买计策强化 | 4 金币，随机计策等级 +1 |
| 删牌 | 从牌组移除 1 张牌，费用每次翻倍 |
| 刷新 | 重新生成牌店库存，费用递增；朋友证可使每次进入商店后的第一次刷新免费 |
| 收缴证 | 持有后购买卡牌免费 |
...
| 词条附魔 | 每次提供两个不重复词条选项；第一次进入铁匠商店后的首次附魔免费 |
| 刷新遗物+词条 | 刷新后词条尽量避开上一组，费用递增；朋友证可使每次进入铁匠后的第一次刷新免费 |

已实现遗物覆盖低/中/高/BOSS 主要效果：每战第一张牌、每回合第一张牌、保留牌首张加成、指定格卡牌加成、拥挤/单格战术倍率加成、两回合击杀金币、战斗开始金币、点金术、小幸运、长线格子成长、训练核心、黄之骨/黄之心/黄之肉、抽牌一号、收缴证、小卡组删牌与大卡组复制。

BOSS 遗物三选一会保证包含当前击败 BOSS 的一件专属遗物；击败黄色君王时必含 `黄之骨` / `黄之心` / `黄之肉` 之一。
