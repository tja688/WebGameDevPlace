...
| **v0.8** | **2026-05-21** | **手感与表现大规模升级：抽卡动画美化（缓动+旋转+拖尾）、补充程序化音效（抽卡/悬停/稀有度/死亡等）、激活视觉特效（放置粒子/伤害飘字/胜利礼花/心损碎裂）、补充鼠标悬停 tooltip（怪物/格子/按钮详细说明）、标题去版本号加暂定标识** |
| **v0.9** | **2026-05-21** | **深化体验优化：堆叠递进音效、攻击屏幕晃动（溢出越大越剧烈）、右上角遗物 persistent 显示+悬停、调试面板改为词条说明书+音量控制、锻造附魔显示词条具体效果、双BGM循环（Airship Serenity平时 / Pixel Peeker Polka Boss战）** |
| **v1.0** | **2026-05-22** | **点对点修复策划提出的问题：生长体系卡牌按第一版正式设计重做（炫耀肌肉赋予生长、训练技巧光环不限生长牌、合理训练赋予生长2、30小时训练/激素训练数值修正、训练纲领改为打出时效果）；怪物数值全面上调匹配设计文档（第一层普通怪75-300、精英200、BOSS 400）；爱护装备修复跨战斗伪永久数值bug；忆往昔改为弃牌堆选定拿牌并新增战斗内选择UI；豪华装备修复同格多张倍率叠加bug** |
| v1.1 | 2026-05-22 | 网页标题显示当前版本号 |
| **v1.2** | **2026-05-23** | **第二版设计全面升级：固定3格牌桌（移除锁定/解锁机制）、抽牌4→5/手牌上限10/保留词条/回合开始洗牌、词条体系重构（新增连携/双生/保留/蔓延/合群/齐心，移除灵动/堆叠/吞噬/征收/复用/驻场/离场）、卡牌数值模型更新（白10/蓝20/金40）、战斗流程改为直接扣减怪物HP、地图6节点/战后流程重构（普通→牌店→事件，精英→铁匠→事件，BOSS→遗物→事件）、遗物稀有度分级（common/rare/epic/boss）、首回合击杀额外+1魂** |
| **v1.3** | **2026-05-23** | **修复BOSS战后黑屏bug（新增boss_relic三选一渲染与输入处理）、事件池系统落地（多选项事件界面+概率框架+8种事件效果）、跨层支持完善（动态格子数3→5、三层关卡配置、act_transition动态文本）、铁匠改为随机强化倍率格、卡牌数值微调（佯攻10→15/炫耀肌肉10→8/回响生长10→20）、铁匠排除BOSS遗物、通关文本动态化** |
| **v2.0** | **2026-05-24** | **严格按照第二版设计文档全面重构：数据层完全重写（卡牌/怪物/职业/关卡/遗物/事件池），核心系统升级（计策系统/8节点地图/金币经济），效果系统更新（移除旧效果/新增专属效果），渲染层适配（计策显示/8节点地图/金币显示），职业统一为老兵，怪物三层8节点，首回合击杀+2金币** |
| **v2.1** | **2026-05-24** | **修复5类BUG：奉献改为只触发一次/连携2正确复制/传令随机抽牌/猛训练集体训练空格生效/商店数值强化+5/铁匠格子固定3格不黑屏/事件池全效果实现/怪物技能全部生效（首牌丢弃/少抽/稳重惩罚/窃魂/闪避）** |
| **v2.2** | **2026-05-24** | **系统性修复7大类问题：牌组抽牌死区（deck前端不可达）/铁匠铺enchantKeywords拼写错误导致黑屏/商店数值强化NAN改为单卡独立计费并新增计策等级提升服务/事件random_strategy_level使用无效旧ID改为真实计策ID并实装fight_monster和fight_elite/传令改为顺序抽牌/猛训练等添加"已在倍率格"判断/双生蔓延衍生牌标记isDerived战后销毁** |
| **v3.0** | **2026-05-24** | **词条效果Playground系统上线：场景执行引擎（浏览器/Node通用）、18个预设测试场景覆盖全部11种词条及边界组合、LocalStorage运行时覆盖层（改数→保存→即时生效→跨会话保留）、DOM编辑面板（JSON编辑/导入/导出）、AI Playtest Harness（window.AITest API，支持run/runAll/runCustom/quickTest/report批量验证）、Canvas简化渲染可视化** |
| **v3.1** | **2026-05-24** | **Playground深度改造为人类友好的对战测试场：复用真实战斗渲染与输入交互、右侧悬浮控制面板（对手选择/血量重置/清空格子/任意卡牌添加手牌/随机抽卡/实时战斗信息）、参数配置面板（卡牌基础数值/怪物HP编辑，保存即生效并持久化到localStorage）、战斗结束自动重置（胜利/失败均自动恢复血量并重新抽牌，不跳屏）、AI词条测试功能完整保留**
| **v3.2** | **2026-05-24** | **参数配置面板独立化：从测试控制台内嵌折叠面板升级为520px居中独立浮层面板，支持开关（按钮/关闭/X/ESC）、搜索过滤（名称/ID实时筛选）、已修改项高亮与原始值显示、单字段恢复默认、回车快捷保存；仅在Playground对战测试场可用**
