...
> 纯前端网页游戏，以仓库内 `design/第二版设计/` 为权威设计镜像（原始来源：`C:\Users\jinji\Desktop\文档\MyNote\游戏开发项目\卡牌地下城\第二版设计`）。每次开发前执行增量 diff 核对，只落地差异。
...
| 训练搭子 | 成长2；当打出成长牌时，自动从牌组打出到同格 |
| 辅助训练 | 已在倍率格后，下一张同格卡牌获得成长1 |
| 30小时训练 | 成长；打出时本牌成长数+1 |
| 激素训练 | 成长2；打出时，每有10点数给所在倍率格+1 |
| 规律训练 | 成长；打出时从牌组拿一张成长牌入手牌 |
| 训练成果 | 成长；本局每打出过一张成长牌，本牌点数+2 |

大数字体系专属效果：

| 卡牌 | 当前实现 |
|---|---|
| 丑陋炫耀 | 打出时，若所在格卡牌总点数>100，抽一张牌 |
| 学徒铸造 | 已在倍率格后，下一张同格卡牌永久+5 |
| 预借 | 打出时，本牌永久-10 |
| 大师铸造 | 已在倍率格后，下一张同格卡牌永久+10 |
| 完美借力 | 打出时，获得相邻两侧最高点数之和的永久点数 |
| 熟练预借 | 打出时，本牌永久-5 |
| 一人成军 | 保留；获得牌组总点数之和的永久点数 |

新增中立牌：

| 卡牌 | 当前实现 |
|---|---|
| 先手优势 | 本回合第一张打出时，点数+5并获得留场 |
| 殿后 | 打出后手牌为空时，点数+10 |
| 战旗 | 打出时，相邻两侧倍率格卡牌点数+5 |
| 顺手的事 | 打出时，获得1金币 |
| 灵活调度 | 打出时，若已触发计策，本格倍率+1 |
...
| v4.5 | 2026-05-25 | 引入 AnimationEngine 动画引擎，实现逻辑层与表现层完全解耦；所有战斗结算通过 effectTimeline 驱动动画队列，支持 skip/reset/speed 控制 |
| v4.6 | 2026-05-25 | 修复动画/音效 bug 并落地新卡牌设计；补充战斗初始化时 AnimationEngine.reset() 调用，防止旧战斗动画污染新战斗 |
| v4.7 | 2026-05-25 | 修复动画系统导致的卡死/手牌丢失恶性 bug（临时方案：恢复 v4.5 输入锁定机制 AnimationEngine.isLocked，禁止动画期间重复触发 endTurn；draw_card 动画增加防御性空值检查） |
| v4.8 | 2026-05-25 | 彻底修复动画期间操作导致的卡死/手牌丢失，同时恢复自由操作体验：移除输入锁定，endTurn 增加 turnTransitioning 防抖；所有抽牌/生成牌/移牌动画改用事件快照（cardSnapshot），不再依赖实时 state.hand 引用 |
...
当前代码严格以设计文档为事实来源。所有超出设计范围的实现已记录在 `REFACTOR_NOTES.md` 中，重构后逐项核对确保无丢失。

### 增量同步工作流

每次开工前，先 diff 外部原始目录与仓库内 `design/第二版设计/` 的差异，同步文档后再扫描代码实现缺口，只落地新增/变更部分。详见 `AGENTS.md` >「设计文档增量同步工作流」。
