...
2. **同步设计文档**：将外部差异同步到 `design/第二版设计/`，确保仓库内副本与外部权威来源一致。

3. **阅读变更内容**：只阅读发生变更的设计文档章节，理解新增需求。

4. **扫描代码实现**：检查当前代码中对应模块的实现状态，找出"设计有但代码无"的差异项。

5. **落地差异**：只实现新增/变更的部分，不重构未涉及的旧代码。

6. **更新 README.md**：如果新增功能影响玩法事实，同步更新 README.md 中的对应表格和描述。

7. **版本号 +0.1 并全量提交**。

> **禁止**：未做 diff 直接全量扫描代码；**禁止**：不同步设计文档就直接改代码。
