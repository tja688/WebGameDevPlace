...
- 2026-06-08：`BattleScene` 旧版补牌依赖多个 `delayedCall + _drawLocked` 分散防重；后触发的补牌请求会被直接丢弃，容易在快速拾取、快速击杀或精英导师弹窗后留下未补的空格。
- 2026-06-08：商店若直接从 `helpDeck` 的卡实例洗牌截断，会天然允许同 `key` 多次出现；删卡页返回商店时若再重洗，还会让“已售出”状态和实际展示错位。
