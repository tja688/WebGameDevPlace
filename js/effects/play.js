...
// ===== 3. 合理训练（proper_training）：同格卡牌获得生长2 =====
FX.register(new EffectHandler({
    id: 'proper_training',
    triggers: Trigger.ON_PLAY,
    ...
    execute: (ctx) => {
        ...
        for (const c of slot.cards) {
            ...
            } else if (c.uuid !== ctx.card.uuid && c.keywords.includes('grow') && c.growAmount < 2) {
                c.growAmount = 2;
...
// ===== 5. 训练痕迹（training_trace）：点数>4连锁打出 =====
FX.register(new EffectHandler({
    id: 'training_trace',
    triggers: Trigger.ON_PLAY,
...
// ===== 11. 炫耀肌肉（show_muscle）：打出时相邻两侧卡牌获得生长1 =====
FX.register(new EffectHandler({
...
// ===== 13. 忆往昔（recall_past）：从弃牌堆选定拿回一张卡牌 =====
FX.register(new EffectHandler({
    id: 'recall_past',
    triggers: Trigger.ON_PLAY,
    priority: Priority.DRAW + 10,
...
// ===== 14. 训练纲领（training_program）：打出时根据训练牌数量生长 =====
FX.register(new EffectHandler({
...
// ===== 15. 爱护装备（surging_anger）：设置下一张同格加成+4 =====
FX.register(new EffectHandler({
    id: 'surging_anger',
    triggers: Trigger.ON_PLAY,
